...
Declaring it as a resource is what makes it manageable across the cluster. The configuration lives in one place instead of in every task definition, so rotating a key, pinning a new model version, or tightening a parameter is one `ax apply`. The default Model also configures the built-in workspace goal bootstrap and is passed to each task command through environment variables.
